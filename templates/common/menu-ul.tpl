			...
			{foreach $data.$pid as $menu}
				...
				{else}
					<li class="m_{$menu.page_name}
						...
						{if (isset($data[$menu.el_id]) || isset($menu_children)) && $menu.level == 0  && $position != 'left'}<span class="navbar-nav__drop dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"></span>{/if}
